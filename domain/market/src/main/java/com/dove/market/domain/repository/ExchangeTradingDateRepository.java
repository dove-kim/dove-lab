...
public interface ExchangeTradingDateRepository extends JpaRepository<ExchangeTradingDate, ExchangeTradingDateId> {
    ...
    /**
     * 거래소·기간 내 개장일 목록을 반환한다.
     */
    List<ExchangeTradingDate> findByOpenTrueAndId_ExchangeAndId_TradeDateBetween(
            Exchange exchange, LocalDate from, LocalDate to);

    /**
     * 거래소·기간 내 개장 + 주가 미수집 날짜 목록을 반환한다.
     */
    List<ExchangeTradingDate> findByOpenTrueAndPricesSyncedFalseAndId_ExchangeAndId_TradeDateBetween(
            Exchange exchange, LocalDate from, LocalDate to);
