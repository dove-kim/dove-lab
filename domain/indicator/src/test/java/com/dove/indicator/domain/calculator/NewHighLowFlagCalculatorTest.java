...
class NewHighLowFlagCalculatorTest {
    ...
    private StockPrice createPrice(int dayOffset, long closePrice) {
        return new StockPrice("005930", StockExchange.KOSPI, PriceType.RAW,
                LocalDate.of(2024, 1, 1).plusDays(dayOffset),
                closePrice, closePrice, closePrice, closePrice, 1000L, null);
    }

    private List<StockPrice> build252PricesWithLastClose(long lastClose) {
    ...
    void shouldReturn1WhenCloseIs52WeekHigh() {
        List<StockPrice> data = build252PricesWithLastClose(9999L);
    ...
    void shouldReturn1WhenCloseIs52WeekLow() {
        List<StockPrice> data = build252PricesWithLastClose(1L);
