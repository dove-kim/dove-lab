...
interface Props {
  ...
  stockFilters: Array<{ id: number; name: string; scope: "SYSTEM" | "MEMBER" }>;
}

export default function FilterEditorModal({ open, onClose, onSaved, initial, stockFilters }: Props) {
  if (!open) return null;
      ...
      <div className="px-6 py-4 border-b border-white/10 flex-shrink-0">
        ...
        <div className="mt-4 pt-4 border-t border-white/5">
          ...
          <Select
            ...
            className="min-w-52"
          />
          <p className="text-xs text-slate-500 mt-1">시스템 필터 또는 본인의 개인 필터를 먼저 적용합니다.</p>
        </div>
      </div>
