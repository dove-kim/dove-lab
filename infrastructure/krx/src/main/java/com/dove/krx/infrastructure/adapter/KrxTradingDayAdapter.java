...
public class KrxTradingDayAdapter implements TradingDayPort {
    ...
    private List<StockListing> fetch(MarketType market, LocalDate date) {
        KrxListedStockResponse response = switch (market) {
            case KOSPI  -> krxStockClient.getKospiListedStocks(props.getAuthKey(), date);
            case KOSDAQ -> krxStockClient.getKosdaqListedStocks(props.getAuthKey(), date);
            case KONEX  -> krxStockClient.getKonexListedStocks(props.getAuthKey(), date);
        };
